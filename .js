                        ...
                        <div className="space-y-2 pt-4 border-t border-white/5">
                            ...
                            {items.some(i => i.steamIron) && (
                                <div className="flex justify-between text-sm text-gray-400">
                                    ...
                                    <span>₹{items.reduce((acc, i) => i.steamIron ? acc + (Number(i.quantity) || 1) * Number(pricing.steamIronPrice) : acc, 0)}</span>
                                </div>
                            )}
                            <div className="flex justify-between text-xl font-black text-white pt-2 border-t border-white/10">
                                <span>Estimated Total</span>
                                <span className="text-green-400">₹{Math.round(estimatedWeight * Number(pricing.pricePerKg)) + items.reduce((acc, i) => i.steamIron ? acc + (Number(i.quantity) || 1) * Number(pricing.steamIronPrice) : acc, 0)}</span>
                            </div>
