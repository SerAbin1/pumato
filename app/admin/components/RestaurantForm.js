        ...
        <div className="bg-white/5 backdrop-blur-xl p-8 md:p-12 rounded-[2.5rem] border border-white/10 max-w-5xl mx-auto shadow-2xl relative">
            ...
            {!isPartnerPage && (
                <div className="border-t border-white/10 pt-10 mb-10">
                    <PreOrderSlotEditor
                        isEnabled={!!formData.isPreOrderEnabled}
                        onToggleEnabled={(next) =>
                            setFormData({ ...formData, isPreOrderEnabled: next })
                        }
                        slots={formData.preOrderSlots || []}
                        onSlotsChange={(preOrderSlots) =>
                            setFormData({ ...formData, preOrderSlots })
                        }
                        label="Pre-order Delivery Slots"
                        emptyText="No pre-order slots defined for this restaurant."
                    />
                </div>
            )}
