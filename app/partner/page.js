            ...
            <div className="p-5 space-y-4">
                ...
                {order.instructions && (
                    <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl px-4 py-3">
                        <p className="text-[10px] font-black text-yellow-400/70 uppercase tracking-widest mb-1">
                    ...
                    {showOosPicker && (
                        ...
                        >
                            ...
                            {restaurantItems.map((item, idx) => (
                                <div
                                    key={idx}
                                    onClick={() => toggleOosItem(item.id || item.name)}
                                    className="flex items-center gap-3 cursor-pointer group"
                                    data-testid={`oos-item-checkbox-${item.id || item.name}`}
                                >
                                    <div
                                        className={`w-5 h-5 rounded-md border flex items-center justify-center transition-all ${
                                            oosItems.includes(item.id || item.name)
                ...
                <div className="max-w-5xl mx-auto px-4 pb-0 flex gap-1">
                    {tabConfig.map((tab) => (
                        ...
                        >
                            ...
                            {tab.badge && (
                                <span
                                    className={`${tab.badgeColor} text-white text-[10px] font-black px-1.5 py-0.5 rounded-full min-w-[18px] text-center`}
                                >
                                    {tab.badge}
                                </span>
                            )}
                        ...
                        {liveOrders.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-24 text-center">
                                ...
                                <p className="text-gray-600 text-sm">
                                    New confirmed orders will appear here automatically.
                                </p>
                            ...
                            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                                {pastOrders.map((order) => {
                                    const restaurantItems =
                                        order.items?.filter(
                                            (i) => i.restaurantId === user.restaurantId
                                        ) || [];
                                    const statusInfo = STATUS_LABELS[order.status] || {
                                        label: order.status,
                                        color: "text-gray-400 bg-gray-500/10 border-gray-500/20",
                                                    ...
                                                    >
                                                        ...
                                                        <span
                                                            className={`text-sm font-medium ${
                                                                ...
                                                                order.outOfStockItems?.includes(
                                                                    item.id || item.name
                                                                )
