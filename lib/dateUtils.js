...
/**
 * Checks if a manual override is present
 * @param {Object} settings - The settings object containing manualOverride.status
 * @returns {string|null} - 'open', 'closed', or null (if auto)
 */
export const checkManualOverride = (settings) => {
    const status = settings?.manualOverride?.status; 
    
    return ["open", "closed"].includes(status) ? status : null;
};
