...
async function deleteUser(accessToken: string, uid: string) {
    const res = await fetch(`https://identitytoolkit.googleapis.com/v1/projects/${FIREBASE_PROJECT_ID}/accounts:delete`, {
        method: "POST",
        headers: { "Authorization": `Bearer ${accessToken}`, "Content-Type": "application/json" },
        body: JSON.stringify({ localId: uid }),
    })
    const deleteResText = await res.text()
    let data;
    try {
...
Deno.serve(async (req) => {
    ...
    try {
        ...
        const { action, email, password, restaurantId, uid } = body
